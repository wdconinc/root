...
class ClassDefGenerateInitInstanceLocalInjector:
   ...
   public:
      ...
      static void Delete(void *p) { if constexpr (std::is_destructible_v<T>) delete ((T*)p); }
      static void DeleteArray(void *p) { if constexpr (std::is_destructible_v<T>) delete[] ((T*)p); }
      static void Destruct(void *p) { if constexpr (std::is_destructible_v<T>) ((T*)p)->~T(); }
